...
describe('RawConfigOps', () => {
  ...
  describe('exportRawConfig()', () => {
    ...
    test('1: Export raw config am', async () => {
      const response = await RawConfigOps.exportRawConfig({ options: { path: '/am/json/node-designer/node-type/' + customNode1._id, overrides: {_id: 'test'}, pushApiVersion: {protocol: '2.1', resource: '2.0'}}, state });
      expect (response._id).toBe('test')
      expect(response).toMatchSnapshot();
    });

    test('2: Export raw config openidm', async () => {                                                                
      const response = await RawConfigOps.exportRawConfig({ options: { path: `/openidm/config/${EMAIL_TEMPLATE_TYPE}/${template1._id}` }, state });
      expect(response).toMatchSnapshot();
    });

    test('3: Export raw config environment', async () => {
      const response = await RawConfigOps.exportRawConfig({ options: { path: '/environment/variables/' + variable1._id }, state });
      expect(response).toMatchSnapshot();
    });
  });
  ...
  describe('importRawConfig()', () => {
    ...
    test(`1: importRawConfig am`, async () => {
      const response = await RawConfigOps.importRawConfig({
        options: {path: '/am/json/node-designer/node-type/' + customNode2._id},
        data: customNode2,
        state
      });
      expect(response).toMatchSnapshot();
