...
async function stageVariable(
  ...
  try {
    ...
    if (create) {
      await VariablesOps.putVariable({
...
describe('VariablesOps', () => {
  ...
  describe('exportVariables()', () => {
    ...
    test('1: Export all variables', async () => {
      const response = await VariablesOps.exportVariables({ state: state });
  ...
  describe('exportVariable()', () => {
    ...
    test('1: Export variable1', async () => {
      const response = await VariablesOps.exportVariable({ variableId: variable1.id, state: state });
      expect(response).toMatchSnapshot({
        meta: expect.any(Object),
      });
    });

    test('2: Export variable2', async () => {
      const response = await VariablesOps.exportVariable({ variableId: variable2.id, state: state });
      expect(response).toMatchSnapshot({
        meta: expect.any(Object),
      });
    });
    ...
    test('3: Export variable3 (non-existent)', async () => {
      ...
      try {
        await VariablesOps.exportVariable({ variableId: variable3.id, state: state })
      } catch (e: any) {
        errorCaught = true;
