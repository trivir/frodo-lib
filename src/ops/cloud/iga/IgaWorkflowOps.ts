...
import {
  ...
  queryRequests,
  RequestSkeleton,
} from '../../../api/cloud/iga/IgaRequestApi';
import {
...
export async function deleteDraftWorkflow({
  ...
  try {
    if (options.force) await cancelPendingWorkflowRequests(workflowId, state);
...
/**
 * Helper that cancels any pending workflow requests for a given workflow
 * @param workflowId the workflow id
 * @returns {Promise<RequestSkeleton[]>} the cancelled requests
 */
async function cancelPendingWorkflowRequests(
  workflowId: string,
  state: State
): Promise<RequestSkeleton[]> {
  const pendingWorkflowRequests = await queryRequests({
    queryFilter: `workflow.id eq "${workflowId}" and decision.status eq "in-progress"`,
    fields: ['id'],
    state,
  });
  const cancelRequests = [];
  for (const r of pendingWorkflowRequests) {
    cancelRequests.push(
      cancelRequest({
        requestId: r.id,
        state,
  ...
  return await Promise.all(cancelRequests);
}
