 ...
 *
 *        FRODO_HOST=frodo-dev npm run test:record EnvReleaseOps
 *
 ...
 *
 *        npm run test:only EnvReleaseOps
 *
 * Note: FRODO_DEBUG=1 is optional and enables debug logging for some output
 * in case things don't function as expected
 */
import * as EnvReleaseOps from './EnvReleaseOps.ts';
...
describe('EnvReleaseOps', () => {
  ...
  describe('readRelease()', () => {
    test('0: Method is implemented', async () => {
      expect(EnvReleaseOps.readRelease).toBeDefined();
    });

    test('1: Get release info - success', async () => {
      const response = await EnvReleaseOps.readRelease({
        state,
      });
