...
export async function importRawConfig({
  ...
  try {
    ...
    switch (startPath) {
      case 'am':
        response = await putRawAm({ endpoint: noStart, payload, apiVersion: options.pushApiVersion, state });
        break;
      case 'openidm':
        response = await putRawIdm({ endpoint: noStart, payload,  state });
        break;
      case 'environment':
        response = await putRawEnv({ endpoint: noStart, payload, apiVersion: options.pushApiVersion, state });
        break;
      default:
        throw new FrodoError(
          `URL paths that start with ${startPath} are not supported`
    ...
    throw new FrodoError(
      `Error in importRawIdm with relative url: ${options.path}`,
      error
    );
  }
}