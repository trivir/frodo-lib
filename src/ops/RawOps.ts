...
export async function exportRawIdm({ state, url }: { state: State; url: string; }): Promise<IdObjectSkeletonInterface> {
    try {
        ...
        throw new FrodoError(`Error in exportRawIdm with relative url: ${url}`, error);
    }
}

export async function exportRawAm({ state, url }: { state: State; url: string; }): Promise<IdObjectSkeletonInterface> {
    try {
        ...
        throw new FrodoError(`Error in exportRawAm with relative url: ${url}`, error);
    }
}

export async function exportRawEnv({ state, url }: { state: State; url: string; }): Promise<IdObjectSkeletonInterface> {
    try {
        ...
        throw new FrodoError(`Error in exportRawEnv with relative url: ${url}`, error);
    }
}