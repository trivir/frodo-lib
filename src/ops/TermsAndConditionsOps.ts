import { restGetTermsAndConditions, TermsAndConditions } from "../api/TermsAndConditionsApi";
...
export async function exportTermsAndConditions({
  ...
  try {
    debugMessage({ message: `TermsAndConditions.exportTermsAndConditions: start`, state });
    const terms = await restGetTermsAndConditions({ state });
    debugMessage({ message: `TermsAndConditions.exportTermsAndConditions: end`, state });
    return terms;
  } catch (error) {
