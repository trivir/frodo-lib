  ...
  try {
    ...
    if (!canSecretStoreHaveMappings(secretStoreTypeId))
      throw new FrodoError(
        `Cannot create mappings for the secret store type '${secretStoreTypeId}'`
      );
    ...
    throw new FrodoError(
      `Error creating mapping '${secretStoreMappingData.secretId}' for the secret store '${secretStoreId}'`,
      error
    );
  }
}
...
export async function readSecretStoreSchema({
  ...
  try {
    ...
    throw new FrodoError(
      `Error reading secret store schema for type ${secretStoreTypeId}`,
      error
    );
  }
}
...
export async function readSecretStoreMapping({
  ...
  try {
    ...
    if (!canSecretStoreHaveMappings(secretStoreTypeId))
      throw new FrodoError(
        `No mappings exist for the secret store type '${secretStoreTypeId}'`
      );
    ...
    try {
      ...
      if (secretStoreMappings) {
        try {
          ...
          result.mappings = await Promise.all(mappings);
        } catch (e) {
          if (resultCallback) resultCallback(e);
        }
      }
