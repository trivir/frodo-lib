...
/**
 * Get request form assignments
 * @param {string} formId The request form id
 * @param {RequestFormType} requestFormType The optional request form type. If not specified, gets all assignments for the form
...
export async function getRequestFormAssignments({
  ...
  const objectIdType =
    requestFormType === 'applicationRequest'
      ? 'application/'
      : requestFormType === 'lcmRequest'
        ? 'lcm/'
  ...
  return await getApiSearchAll<RequestFormAssignment>({
    url: urlString,
    queryFilter: `objectId co "${objectIdType}" and formId eq "${formId}"`,
    state,
  });
}
