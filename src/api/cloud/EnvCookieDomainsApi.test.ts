 ...
 *
 *        FRODO_RECORD_PHASE=1 FRODO_HOST=frodo-dev npm run test:record EnvCookieDomainsApi
 *        FRODO_RECORD_PHASE=2 FRODO_HOST=frodo-dev npm run test:record EnvCookieDomainsApi
 *
 ...
 *    by running:
 *
 *        FRODO_DEBUG=1 npm run test:update EnvCookieDomainsApi
 *
 * 3. Test your changes
 *
...
describe('EnvCookieDomainsApi', () => {
  ...
  if (
    ...
    describe('getCookieDomains()', () => {
      test('0: Method is implemented', async () => {
        expect(EnvCookieDomainsApi.getCookieDomains).toBeDefined();
      });

      test('1: Get cookie domains - success', async () => {
        const response = await EnvCookieDomainsApi.getCookieDomains({
          state,
        });
  ...
  if (
    ...
    describe('setCookieDomains()', () => {
      test('0: Method is implemented', async () => {
        expect(EnvCookieDomainsApi.setCookieDomains).toBeDefined();
      });

      test(`1: Set cookie domains - success`, async () => {
        const response = await EnvCookieDomainsApi.setCookieDomains({
          domains: testDomains,
          state,
        });
        expect(response).toMatchSnapshot();
