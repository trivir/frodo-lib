...
const AM_ENTITIES: Record<string, EntityInfo> = {
  ...
  // Same thing happens in Amster and the AM API explorer as of version 7.5.0. In the future, if this endpoint gets fixed, uncomment the following code
  /*sessions: {
...
function getApiConfig(version: string) {
  return {
    apiVersion: `protocol=2.1,resource=${version}`,
  };
}
