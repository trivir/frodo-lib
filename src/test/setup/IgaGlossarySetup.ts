  ...
  beforeEach(async () => {
    if (process.env.FRODO_POLLY_MODE === 'record') {
      ctx.polly.server.any().on('beforePersist', (_req, recording) => {
        ...
        // Replace ids
        for (const glossary of allGlossaries) {
          const oldId = oldGlossaryIds.get(glossary.id);
          ...
          if (recording.request.postData)
            recording.request.postData.text =
              recording.request.postData.text.replaceAll(glossary.id, oldId);
          recording.response.content.text =
            recording.response.content.text.replaceAll(glossary.id, oldId);
        }
      });
      // Normalize id's from created ids to test ids
      ctx.polly.config.matchRequestsBy.url.pathname = (path) => {
        const glossaryId = oldGlossaryIds.keys().find((k) => path.endsWith(k));
        if (!glossaryId) return path;
        return path.replaceAll(glossaryId, oldGlossaryIds.get(glossaryId));
      };
      ctx.polly.config.matchRequestsBy.body = (bodyString) => {
        const glossaryId = JSON.parse(bodyString).id;
        const glossaryValue = oldGlossaryIds.get(glossaryId);
        if (!glossaryValue) return bodyString;
        return bodyString.replaceAll(glossaryId, glossaryValue);
      };
      ctx.polly.server.any().on('beforeResponse', (req, res) => {
        const glossaryIds = [];
        const pathId = oldGlossaryIds
          .keys()
          .find((k) => req.pathname.endsWith(k));
        if (pathId) {
          glossaryIds.push(pathId);
        } else {
          const body = JSON.parse(res.body);
          if (body.id) {
            glossaryIds.push(body.id);
          } else if (
            Array.isArray(body.result) &&
            body.result.length > 0 &&
            // Since glossary6 is used for deletes, where there are multiple requests, we can't update the response here for that case
            !req.body.includes(glossary6.name)
          ) {
            for (const data of body.result) {
              if (data.id) glossaryIds.push(data.id);
            }
          }
        }
        for (const glossaryId of glossaryIds) {
          const glossaryValue = oldGlossaryIds.get(glossaryId);
          if (glossaryValue) {
            res.body = res.body.replaceAll(glossaryId, glossaryValue);
          }
        }
      });
    }
